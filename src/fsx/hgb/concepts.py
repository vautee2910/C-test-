...
def classify_document_family(text: str) -> str:
    """Classify a document into a family name from the registry.

    Returns the matched family ("hgb", "bank", …) or "unknown" when nothing
    qualifies. Thin wrapper over :data:`fsx.hgb.families.REGISTRY`; the markers,
    thresholds and concept sets live in ``config/families.yaml`` so new document
    types are added as data. Used to pick the concept set so a family's positions
    are only offered to documents of that family.
    """
...
def concept_paths_for_family(family: str) -> tuple[Path, ...]:
    ...
    Bank statements load HGB + bank; an industrial sheet loads HGB only;
    "unknown" falls back to the base concepts (best-effort). Defined by
    ``config/families.yaml``.
    """
    from .families import REGISTRY

    return REGISTRY.by_name(family).concept_paths
